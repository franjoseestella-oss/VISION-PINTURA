// RAL Classic color map → hex values
// Covers the most common RAL colors used in industrial painting
...
export const RAL_COLORS: Record<string, string> = {
    ...
    '7034': '#8F8B66', '7035': '#D7D7D7', '7036': '#7F8274', '7037': '#7D7F7D',
    ...
    '8004': '#8E402A', '8007': '#59351F', '8008': '#6F4F28', '8010': '#866633', // Changed: correct hex
    '8011': '#5B3A29', '8012': '#592321', '8014': '#382C1E', '8015': '#633A34',
    '8016': '#4C2F27', '8017': '#45322E', '8019': '#403A3A', '8022': '#212121',
    '8023': '#A65E2E', '8024': '#79553D', '8025': '#755C48', '8028': '#4E3B31',
    '8029': '#763C28',

    // RAL 9000 series — Whites/Blacks
    '9001': '#FDFBD3', '9002': '#E7EBDA', '9003': '#F4F4F4', '9004': '#282828',
    '9005': '#0A0A0A', '9006': '#A5A5A5', '9007': '#8F8F8F', '9010': '#FFFFFF',
    '9011': '#1C1C1C', '9012': '#FFFDE6', '9016': '#F7F7F7', '9017': '#1E1E1E',
    '9018': '#D7D7D7', '9022': '#9C9C9C', '9023': '#828282',
};

/**
 * Extract RAL code from a barcode string.
 * Supports formats: "RAL 9005", "RAL9005", "9005", "RAL-9005"
 */
export function extractRAL(barcode: string): string | null {
    const cleaned = barcode.trim().toUpperCase();
    // Try "RAL 1234" or "RAL1234" or "RAL-1234"
    const match = cleaned.match(/RAL[\s\-]?(\d{4})/);
    if (match) return match[1];
    // Try standalone 4-digit number that matches a known RAL
    const numMatch = cleaned.match(/^(\d{4})$/);
    if (numMatch && RAL_COLORS[numMatch[1]]) return numMatch[1];
    return null;
}

/**
 * Get hex color for a RAL code. Returns null if not found.
 */
export function ralToHex(ralCode: string): string | null {
    const code = ralCode.replace(/^RAL[\s\-]?/i, '').trim();
    return RAL_COLORS[code] || null;
}
